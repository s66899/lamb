...
def add(lst, n, coach, date, wd):
    for i in range(n):
        ...
        s["id"] = s.get("id","")
        s["name"] = s.get("name","未知")
        att = {
            "id": nid(), "student_id": s["id"], "student_name": s["name"],
...
for d,w,c,s in STATS:
    wd = WD[dt.strptime(d,"%Y-%m-%d").weekday()]
    add(wang, w, "王教练", d, wd)
    add(chen, c, "陈教练", d, wd)
    add(sun,  s, "孙教练", d, wd)
...
print(f"Done. Total: {sum(w+c+s for _,w,c,s in STATS)} sessions, {cnt-1} records added.")
