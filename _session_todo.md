...
- **competition + nutrition 两本仅 5.3k/5.8k 字,内容单薄**(每章 3-5KB);按 ch06-back/ch07-achilles 双层结构 + ex-lib 引用基线标准看,这两本欠 4 倍深度。下一项最有价值的小改进:挑一本最弱的(如 competition ch03-opponent-analysis.md 仅 5 个 h2 + 3491B / 3.5KB)做双层扩写 + 加 5-10 条 [ex:XXXX] 引用。可独立 commit,工作量适中
- **VERSION 头部 v3.22.53 已滞后于最新 commit v3.22.55(本轮)** + APP_VERSION(app.js)仍为 v3.22.53 — 真正的版本号与文档不一致;下轮候选:跑 `node _bump_version.js --apply` 一次性追平 4 埋点 v3.22.53 → v3.22.55(跳 54?)+ 同时 VERSION 头部 HEAD/APP_VERSION 同步更新
- **manifest.json 其他 5 本书 title / 完整性同步**:本轮已修 badminton-recovery(v3.22.44)。剩余候选:NSCA 9/10 章 title 与 manifest_data.js 不一致(如 ch10:manifest_data.js「恢复训练(待扩展)」是过时文案、manifest.json「恢复策略(v3.22.16 双层结构 + 12 ex-lib 已校对)」才是正确 v3.22.16 状态);yin-yang / badminton / engineering-mechanics / finance / psychology 5 本书 manifest.json title 字段未与 manifest_data.js 逐本比对
...
- **books/{competition,nutrition} 两本整书注册到 manifest.json + manifest_data.js** —— **已兑现(v3.22.51 / 本轮已确认)**,manifest.json 9 本 / manifest_data.js 9 本 / books/README.md 9 本表 全部一致,可勾掉
- **books/{competition,nutrition,exercises} 三本书目录真实存在但两份 manifest 都未注册(潜在 UI 大 bug)** —— **本轮已确认实际是 9 本全部注册**;exercises/ 是 ex-lib.json 库的真实存放目录(不是书),无需注册。可勾掉
- 羽毛球康复 ch03 / ch04 / ch05 inline 引用 vs 库内动作名详细比对(id 全部合法,但中文译名一致性未逐条复核)
