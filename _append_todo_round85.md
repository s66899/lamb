# 第 85 轮 ledger — index.html 3 处 ASCII `??` 占位符 → 对应 emoji 清理

**本轮做了什么**：

扫描全项目时发现 `index.html` 有 3 处 HTML / JS 注释里残留 ASCII 双问号 `??` 占位符，是 v3.7.8
(2026-07-06) `feat(device-tracker)` commit 85d3cc7 引入密码层 / 设备管理时的 UTF-8 emoji 字符丢失：
...
3 处原本意图与下方实际 emoji 渲染一致（密码锁 🔐 / 管理员设置 ⚙️），但因 v3.7.8 当时编码失误丢失，
留下 ASCII 双问号 `??` 视觉残留。
