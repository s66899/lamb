...
function renderDashboard() {
  ...
  // v3.22.14 移除了 v3.22.13 的首页 banner（改放左侧工具栏子项，避免重复）
...
function renderToolItems() {
  return [
    ...
    `<div class="side-link sub" onclick="openFitnessQuestionnaire()"><span class="sl-icon">🏋️</span> 问卷训练方案</div>`,
  ].join('');
}
...
async function renderChapter() {
  ...
  if (md) {
    ...
    // FITNESSFORMPLACEHOLDERMARKER 占位（无下划线，避开 mdParse 的 /_(.+?)_/g em 规则；上轮 _FORM_ 这种名字仍被吃）。在 app.js 里后置注入 DOM + 加载 Chart.js（CDN 懒加载，首次访问才下载）。
...
// FITNESSFORMPLACEHOLDERMARKER；本函数读到该占位符后，注入 form DOM + 加载 Chart.js（CDN 懒加载）。
...
function injectFitnessTrackerForm() {
  ...
  // 把占位段落替换成 form DOM。占位段落经 mdParse 渲染为 <p>FITNESSFORMPLACEHOLDERMARKER</p>
  const article = document.getElementById('article');
  if (!article) return;
  const ps = article.getElementsByTagName('p');
  for (let i = 0; i < ps.length; i++) {
    if (ps[i].textContent.indexOf('FITNESSFORMPLACEHOLDERMARKER') >= 0) {
...
function attachFitnessBtnToTools() {
  /* v3.22.14 禁用 banner 挂载，统一走左侧工具栏子项入口 */
  return;
  /*
